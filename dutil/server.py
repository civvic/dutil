# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/00_server.ipynb.

# %% auto #0
__all__ = ['kill_port', 'is_serving', 'wrap_endp', 'find_server', 'setup_uvi_logging', 'local_server']

# %% ../nbs/00_server.ipynb #92031bb1
...
# %% ../nbs/00_server.ipynb #48df1e1e
def find_server(port=8000):
    "Scan IPython user_ns for a JupyUvi instance on `port`"
    import IPython
    ns = IPython.get_ipython().user_ns
    for v in ns.values():
        if isinstance(v, JupyUvi) and v.port == port and v.server and v.server.started and not v.server.should_exit: return v
    return None

# %% ../nbs/00_server.ipynb #3e47ca82
def log(msg, *args, **kwargs): print(f"[{datetime.now():%H:%M:%S}] {msg}", flush=True)

# %% ../nbs/00_server.ipynb #d544286b
