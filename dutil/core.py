...
# %% ../nbs/00_core.ipynb #3a1d4063
_ts, _te = ('<!-- ', '#| '), (' -->', '')
def get_tag(name:str, val:str='', msg_type:str='note'):
    return f"{_ts[msg_type=='code']}{name}: {val or find_msg_id()}{_te[msg_type=='code']}"
...
def find_var(var:str, default:Any=Parameter.empty, raiseex:bool=False):
    try: return _find_var(var)
    except Exception:
        if raiseex: raise
        return default
